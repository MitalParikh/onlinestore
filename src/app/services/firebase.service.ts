...
export class FirebaseService {
    ...
    private _eStoreColl: string = "onlinestore/elish/admins";
    private _eStoreUrl: string = "onlinestore/elish";
    authState: any = null;
    ...
    getDoc(coll: string, docId: string) {
        return this.afs.collection(coll).doc<any>(docId).valueChanges();
    }
	...
	const authenticatedUser = await this.getUser();
      ...
      /*const authenticated = authenticatedUser.pipe(
		map(authState => {
		  if(!authState) {
			return false;
		  } else {
    ...
    isUserAdmin(){
	let docId = !this.isUserLoggedIn()? "abcd" : this.authState.uid;
	let docUrl = "onlinestore/elish/admins/" + docId;
	return this.afs.doc<any>(docUrl).valueChanges();
	
	/*let isAdmin: Observable<boolean> =  uid.pipe(
		map(uid => {
			
			if(uid === null) {
				console.log("hi: You are not authorised Admin, " + uid);
				return false;
			} else {
				console.log("we are here");
				if(! (uid.isadmin)) 
				{
					console.log("hello: You are not authorised Admin, " + uid);
					return false; 
				} else return true;
			}
		})
